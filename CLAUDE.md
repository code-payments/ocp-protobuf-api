...
### Balance Service (2 RPCs)
- `GetBalance` - core mint balances for any owner account, optionally filtered by a repeated `mints` list (empty means all mints the owner holds). Unauthenticated (no `signature` field) since balances are public blockchain state, so it can be called against any public key. Returns the aggregate `core_mint_value` in quarks plus `balances_by_mint` (map of mint address to `MintBalance` with the per-mint `core_mint_value`), with `Result` of OK/DENIED/NOT_FOUND. Likely to be deprecated in favour of `GetBalances`.
- `GetBalances` - batching variant of `GetBalance` for a set of `owners` (1-1024), with the same optional `mints` filter applied to every owner. Also unauthenticated. Returns `balances_by_owner` (map of owner address to `OwnerBalance`, which carries the owner's aggregate `core_mint_value` and its own `balances_by_mint`), with `Result` of OK/DENIED.
